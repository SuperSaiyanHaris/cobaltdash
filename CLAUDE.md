...
- Newsletter / long-form platform. The category leaderboard API exposes a **precise total subscriber count** for ~88% of pubs via the `freeSubscriberCount` field (e.g. `"2,900,000"`). The remaining ~12% only expose the coarse order-of-magnitude band (`rankingDetailFreeIncludedOrderOfMagnitude`: 1K/10K/100K/1M), used as a fallback floor. ALWAYS prefer `freeSubscriberCount` — parse digits, fall back to the band. (Do NOT use `rankingDetailFreeIncludedOrderOfMagnitude` as the primary; that was the original mistake — it made Letters from an American read 1M instead of its real 2.9M.)
...
- `subscribers` = precise total (free+paid) from `freeSubscriberCount`, band floor fallback. `total_views`/`total_posts` = null (no public post count in the leaderboard data — Substack shows just subscribers, no second metric). Substack natively uses **"subscribers"** so it joins YouTube's label group (NOT followers).
- **Ranking is by precise `subscribers` DESC, like every other platform.** `creators.leaderboard_rank` is still populated (global position across category leaderboards) but is only a TIEBREAKER now: `refresh_rankings_cache_platform`'s substack ORDER BY is `stat_subs DESC, leaderboard_rank ASC`. The `subsFor(pub)` helper (precise-or-band) is duplicated in `seedTopSubstackCreators.js`, `discoverSubstackCreators.js`, and `collectDailyStats.js#buildSubstackRanking`.
...
- Profile hero stat shows the precise count via `formatNumber()` (e.g. "2.9M"), like other platforms. Single stat card (subscribers only — no second metric exists). No growth cards. No live counter.
- **Rankings second column:** the per-platform `SECONDARY_COLUMN` map in `Rankings.jsx` decides the one extra column (youtube→Views, tiktok→Likes, music→Plays, rumble→Videos, mastodon/bluesky→Posts; twitch/kick/substack→none). Don't reintroduce ad-hoc per-platform conditions for that column — extend the map.
...
- `rumble.com` and `substack.com` return 403/Cloudflare-challenge to GitHub Actions datacenter IPs. Every fetch fails: Rumble logs "Channel not found" for ALL channels (even PragerU/Timcast), Substack logs "Leaderboard returned 0 ranked publications". The cloud run *completes successfully* but silently writes ~zero rows for these two, which is how every Rumble/Substack profile got stuck on "1 day tracked." The API-based platforms (YouTube/Twitch/Kick/Bluesky/Mastodon/Music) are unaffected — they don't IP-block.
- **The block is per-IP-reputation (datacenter ASN), not per-request.** Better headers/retries don't help. We probed each runtime empirically (2026-06-02):
  - **Substack** is reachable from **Supabase Edge Functions** (200 OK) — Substack does NOT block Supabase's egress IPs. So Substack is now collected **fully in the cloud**: Edge Function `supabase/functions/collect-substack` (Deno port of `buildSubstackRanking` — precise `freeSubscriberCount` else band floor; never writes 0) on a **daily `pg_cron` job `collect-substack-daily` (`0 8 * * *`)** that `net.http_post`s the function with an `x-cron-key` header (secret stored via `supabase secrets set CRON_KEY`; function deployed `--no-verify-jwt` + checks the header). To re-run manually: `curl -X POST .../functions/v1/collect-substack -H "x-cron-key: <CRON_KEY>"`.
  - **Rumble** is blocked from **every** datacenter we tried (GitHub Actions, Vercel, AND Supabase Edge — Cloudflare "Just a moment…" challenge). So Rumble can ONLY be collected from a **residential IP via a local scheduled task** (`scripts/local/collect-rumble.bat`, `COLLECT_ONLY=rumble`). NOT optional — without it, Rumble stops updating. The only cloud alternative would be a residential proxy / scraping API (paid).
- `collectDailyStats.js` **skips rumble + substack when `process.env.GITHUB_ACTIONS==='true'`** and supports `COLLECT_ONLY="rumble"` (etc.) to run a subset locally. Substack is no longer collected by the Node script in normal operation (the Edge Function owns it); the Node path still works for manual `COLLECT_ONLY=substack` runs if ever needed.
- Do NOT "re-enable" rumble/substack in the CI run thinking it's a bug — it's a deliberate skip. If you add another scrape-based platform, probe whether its site 403s each runtime (GH Actions / Supabase Edge) before relying on the cloud.
...
- **`collect-rumble.bat`** — **REQUIRED daily task** (Rumble can't run in any datacenter; see above). Runs `COLLECT_ONLY=rumble node scripts/collectDailyStats.js` + refreshes the rankings cache from a residential IP. (Substack is cloud now — Edge Function + pg_cron.)
