...
export default function PageHero({ eyebrow, title, subtitle, children, aside, center = false, compact = false, className = '' }) {
  return (
    <section className={`relative isolate z-20 bg-[#0a0a0f] text-white ${className}`}>
      ...
      <div
        className={`relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 ${compact ? 'pt-8 pb-8 sm:pt-12 sm:pb-12' : 'pt-8 pb-9 sm:pt-16 sm:pb-16'} ${
      ...
      >
        <div className={center ? 'text-center max-w-3xl mx-auto' : 'max-w-3xl'}>
          ...
          <h1 className={`mt-3 font-extrabold tracking-tight leading-[1.05] text-balance ${compact ? 'text-3xl sm:text-5xl' : 'text-[34px] sm:text-5xl md:text-6xl'}`}>
            {title}
          </h1>
          {subtitle && (
            <p className={`mt-3 sm:mt-4 text-[15px] sm:text-lg text-white/75 leading-relaxed text-pretty ${center ? 'mx-auto max-w-2xl' : 'max-w-2xl'}`}>{subtitle}</p>
          )}
